...
type ServiceBillRepository struct {
	*database.BaseRepository[model2.ServiceBill]
}

func NewServiceBillRepository(db *gorm.DB) *ServiceBillRepository {
	return &ServiceBillRepository{
		BaseRepository: database.NewBaseRepository[model2.ServiceBill](db),
	}
}
...
func (r *ServiceBillRepository) ExistsByNumber(ctx context.Context, number string) (bool, error) {
	var count int64
	err := r.GetDB(ctx).Model(&model2.ServiceBill{}).Where("number = ?", number).Count(&count).Error
	return count > 0, errs.Wrap(err)
}

// FindFullById 查询完整实体
func (r *ServiceBillRepository) FindFullById(ctx context.Context, id uint) (*model2.ServiceBill, error) {
	var res model2.ServiceBill
...
func (r *ServiceBillRepository) FindByParam(ctx context.Context, param *model2.ServiceBillQueryParam) (*result.PageResult[model2.ServiceBill], error) {
	q := r.GetDB(ctx).WithContext(ctx).Model(&model2.ServiceBill{})
	if param.Number != "" {
	...
	var serviceBills []model2.ServiceBill
...
// Updates 更新
func (r *ServiceBillRepository) Updates(ctx context.Context, entity *model2.ServiceBill) error {
	return errs.Wrap(r.GetDB(ctx).Session(&gorm.Session{FullSaveAssociations: true}).Omit("ProductCompany").Save(entity).Error)
}

// Delete 删除整个实体
func (r *ServiceBillRepository) Delete(ctx context.Context, entity *model2.ServiceBill) error {
	return errs.Wrap(r.GetDB(ctx).Select("Details").Delete(entity).Error)
}

// CountByState 根据单据状态计数
func (r *ServiceBillRepository) CountByState(ctx context.Context, states []model2.ServiceBillState) (*model2.CountByStateResult, error) {
	rows := make([]model2.CountByStateRow, 0)
	...
	res := make(model2.CountByStateResult)
	for _, r := range rows {
		state := model2.ServiceBillState(r.State)
...
func (r *ServiceBillRepository) SumReceiveAmountByMonth(ctx context.Context) ([]model2.MonthSumAmount, error) {
	var qRes []model2.YearMonthSumAmount
	now := time.Now()
	...
	err := r.GetDB(ctx).
		...
		Table("service_bill").
		Where("state IN ?", []model2.ServiceBillState{model2.ServiceBillStateProcessed, model2.ServiceBillStateFinished}).
	...
	var res []model2.MonthSumAmount
	for _, row := range qRes {
		res = append(res, model2.MonthSumAmount{
